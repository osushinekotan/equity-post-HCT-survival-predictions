...
def load_data(config, valid_ratio: float | None = None) -> pl.DataFrame:
    ...
    # event=0 のうち、 time が大きいもの top1% を除外
    raw_train_df = pl.read_csv(config.COMP_DATASET_DIR / "train.csv").with_columns(
        pl.lit("TRAIN").alias(config.DATASET_COL),
        pl.lit(-1).alias(config.FOLD_COL),
    )
    # event=0 のうち、 time が大きいもの top1% を除外
    _df = raw_train_df.with_columns(
        pl.col(config.SURVIVAL_TIME_COL).rank(descending=True).over(partition_by=config.EVENT_COL).alias("rank")
    )
    raw_train_df = pl.concat(
        [
            _df.filter(pl.col(config.EVENT_COL) == 0).filter(pl.col("rank") > (_df[config.EVENT_COL] == 0).sum() * 0.1),
            _df.filter(pl.col(config.EVENT_COL) == 1),
        ]
    )
