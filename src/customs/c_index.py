# https://www.kaggle.com/code/adaubas/cibmtr-faster-concordance-index-metric-with-numba
import numpy as np
from numba import jit


@jit(nopython=True)
def init_BTree(values):  # noqa
    ...
    while True:
        ...
        if has_more_censored and (not has_more_died or died_truth[died_ix] > censored_truth[censored_ix]):
            pairs, correct, tied, next_ix = handle_pairs(
                censored_truth, censored_pred, censored_ix, times_to_compare, counts
            )
            censored_ix = next_ix
